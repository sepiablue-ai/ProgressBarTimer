...
- Automatically saves window position, size, and timer length to `timer_config.ini`
...
- Button images are embedded into the EXE, so the `assets` folder is not required at runtime.
- On first launch, `timer_config.ini` is created or updated in the same folder.

For normal distribution, `timer.exe` is enough. Include `timer_config.ini` only if you want to ship a fixed default setting.
...
timer_config.ini                 User settings, ignored by Git
